...
namespace bclibc
{
     ...
     * usage pattern (including its thread-safety notes -- tolerances and
     * stats here are likewise each a `std::atomic`, so one instance can be
     * shared across threads via `std::ref`); assigning the free function
