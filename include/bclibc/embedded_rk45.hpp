...
namespace bclibc
{
    namespace embedded_rk45_detail
    {
        ...
        // Templated on <Tableau, Controller> so each method (Cash-Karp,
        // Dormand-Prince, ...) gets its own independent thread-local state --
        // a plain namespace-scope thread_local would be shared by every
        // instantiation of integrate_embedded_rk45, letting one method's
        // tolerance/stats silently leak into another's.
        template <typename Tableau, typename Controller>
        struct State
        {
            static thread_local int accepted_steps;
            static thread_local int rejected_steps;
            static thread_local double relative_tolerance;
            static thread_local double absolute_tolerance;
        };

        template <typename Tableau, typename Controller>
        thread_local int State<Tableau, Controller>::accepted_steps = 0;
        template <typename Tableau, typename Controller>
        thread_local int State<Tableau, Controller>::rejected_steps = 0;
        template <typename Tableau, typename Controller>
        thread_local double State<Tableau, Controller>::relative_tolerance = default_tolerance;
        template <typename Tableau, typename Controller>
        thread_local double State<Tableau, Controller>::absolute_tolerance = default_tolerance;
    ...
    template <typename Tableau, typename Controller>
    inline void embeddedRKGetStats(int &accepted, int &rejected)
    {
        using S = embedded_rk45_detail::State<Tableau, Controller>;
        accepted = S::accepted_steps;
        rejected = S::rejected_steps;
    }

    template <typename Tableau, typename Controller>
    inline void embeddedRKSetRelativeTolerance(double tolerance)
    {
        if (!std::isfinite(tolerance) || tolerance <= 0.0)
            throw std::invalid_argument("embedded RK relative tolerance must be finite and positive");
        embedded_rk45_detail::State<Tableau, Controller>::relative_tolerance = tolerance;
    }

    template <typename Tableau, typename Controller>
    inline void embeddedRKSetAbsoluteTolerance(double tolerance)
    {
        if (!std::isfinite(tolerance) || tolerance < 0.0)
            throw std::invalid_argument("embedded RK absolute tolerance must be finite and non-negative");
        embedded_rk45_detail::State<Tableau, Controller>::absolute_tolerance = tolerance;
    }

    /** Generic compile-time adaptive embedded Runge--Kutta trajectory core. */
    template <typename Tableau, typename Controller>
    void integrate_embedded_rk45(BCLIBC_BaseEngine &eng,
                                 BCLIBC_BaseTrajDataHandlerInterface &handler,
                                 BCLIBC_TerminationReason &reason)
    {
        using namespace embedded_rk45_detail;
        using S = State<Tableau, Controller>;
        ...
        S::accepted_steps = S::rejected_steps = 0;
        ...
        while (reason == BCLIBC_TerminationReason::NO_TERMINATE)
        {
            ...
            for (int attempt = 0; attempt < max_retries; ++attempt)
            {
                ...
                const double svx = S::absolute_tolerance + S::relative_tolerance * std::max(std::abs(vr.x), std::abs(vr_next.x));
                const double svy = S::absolute_tolerance + S::relative_tolerance * std::max(std::abs(vr.y), std::abs(vr_next.y));
                const double svz = S::absolute_tolerance + S::relative_tolerance * std::max(std::abs(vr.z), std::abs(vr_next.z));
                const double spx = S::absolute_tolerance + S::relative_tolerance * std::max(std::abs(pos.x), std::abs(pos_next.x));
                const double spy = S::absolute_tolerance + S::relative_tolerance * std::max(std::abs(pos.y), std::abs(pos_next.y));
                const double spz = S::absolute_tolerance + S::relative_tolerance * std::max(std::abs(pos.z), std::abs(pos_next.z));
