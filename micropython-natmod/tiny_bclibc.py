...
# Buffer is packed once at construction and held in a 1-field namedtuple.
# API calls receive shot._buf directly — no re-allocation per call.
_Shot = _namedtuple("Shot", ("_buf",))
...
_Request = _namedtuple("Request", ("_buf",))
...
# ── API wrappers ──────────────────────────────────────────────────────────────
version          = _m.version
calculate_energy = _m.calculate_energy
get_correction   = _m.get_correction
calculate_ogw    = _m.calculate_ogw

def integrate(shot, req):             return _m.integrate(shot._buf, req._buf)
def integrate_at(shot, interp, val):  return _m.integrate_at(shot._buf, interp, val)
def integrate_stream(shot, req, cb):  return _m.integrate_stream(shot._buf, req._buf, cb)
def find_zero_angle(shot, dist_ft):   return _m.find_zero_angle(shot._buf, dist_ft)
def find_apex(shot):                  return _m.find_apex(shot._buf)
def find_max_range(shot, lo, hi):     return _m.find_max_range(shot._buf, lo, hi)
