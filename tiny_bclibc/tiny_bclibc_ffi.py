...
Usage::
    ...
    import tiny_bclibc_ffi as bc            # instead of: import tiny_bclibc as bc
...
if struct.calcsize("P") != 8:
    raise ImportError("tiny_bclibc_ffi requires x64 (8-byte pointer) MicroPython")
...
# ── .so location ──────────────────────────────────────────────────────────────
_SO = os.getenv("TINY_BCLIBC_SO") or _rel("build-shared/libtiny_bclibc.so")
_lib = ffi.open(_SO)
...
_f_maxrange = _lib.func("i", "tiny_bclibc_find_max_range", "PddPP")
_f_err = _lib.func("s", "tiny_bclibc_last_error", "")

# ── C struct sizes (x64, dp) ──────────────────────────────────────────────────
_SHOT_C_SIZE = 232  # TINY_BCLIBC_Shot
_PROPS_C_SIZE = 512  # TINY_BCLIBC_ShotProps (actual ~376, 512 for safety)
_TRAJ_C_SIZE = 128  # TINY_BCLIBC_TrajectoryData: 15d + i32 + 4pad
_BASE_C_SIZE = 64  # TINY_BCLIBC_BaseTrajData: 8d
_REQ_C_SIZE = 32  # TINY_BCLIBC_TrajectoryRequest: 3d + i32 + pad
_WIND_C_SIZE = 32  # TINY_BCLIBC_Wind: 4d
_CURVE_C_SIZE = 32  # TINY_BCLIBC_CurvePoint: 4d


# ── Get pointer address of a buffer ──────────────────────────────────────────
def _ptr(buf):
    return uctypes.addressof(buf)


# ── Built-in drag tables (double) — matches bclibc_mp.c ──────────────────────
_G7_MACH = array.array(
    "d",
    [
        0.00,
        0.05,
...
# ── Python-side buffer sizes (float32 packed) — mirrors tiny_bclibc.py ───────
_NaN = float("nan")
_INF = 1e8
_MAX_WINDS = 16
_MAX_DRAG_PTS = 128
...
_CFG_SIZE = 28
_REQ_SIZE = 16
_SHOT_HOLDER_SIZE = 1  # not used by ffi; kept for API compatibility
_TRAJ_DATA_SIZE = 128  # dp: 15d + i32 + 4pad
...
# ── Wind ──────────────────────────────────────────────────────────────────────
_Wind = _namedtuple("Wind", ("_buf", "_s"))
...
# ── Config ────────────────────────────────────────────────────────────────────
_Config = _namedtuple("Config", ("_buf", "_s"))
...
# ── Shot ──────────────────────────────────────────────────────────────────────
_Shot = _namedtuple("Shot", ("_buf", "_s", "_holder"))
...
def Shot(
    ...
    off = _SHOT_SIZE
    for i in range(wc):
        buf[off : off + _WIND_SIZE] = winds[i]._buf
        off += _WIND_SIZE
...
# ── Request ───────────────────────────────────────────────────────────────────
_Request = _namedtuple("Request", ("_buf", "_s", "_traj"))
...
def _build_props(shot_buf):
    """Build TINY_BCLIBC_ShotProps from a shot._buf (float32 packed).

    Mirrors bclibc_mp.c build_props_buf() field-by-field.
    Returns (props_c, holder) where holder keeps pointer targets alive.
    """
    buf = shot_buf
...
def _pack_req_c(req):
    """Convert Request (float32 buf) to TINY_BCLIBC_TrajectoryRequest (dp C struct)."""
    limit, step, tstep = struct.unpack_from("<3f", req._buf, 0)
    flags = struct.unpack_from("<i", req._buf, 12)[0]
    req_c = bytearray(_REQ_C_SIZE)
    struct.pack_into("<3d", req_c, 0, float(limit), float(step), float(tstep))
    struct.pack_into("<i", req_c, 24, flags)
    return req_c
...
def integrate(shot, req):
    props_c, _alive = _build_props(shot._buf)
    req_c = _pack_req_c(req)
    cap = len(req._traj) // _TRAJ_C_SIZE
    out_w = bytearray(4)
    out_t = bytearray(4)
    out_r = bytearray(4)
    rc = _f_integ(props_c, req_c, req._traj, cap, out_w, out_t, out_r)
    if rc == 6:  # ERR_BUF_TOO_SMALL — allocate exact size
    ...
    written = struct.unpack("<i", out_w)[0]
    reason = struct.unpack("<i", out_r)[0]
    return [_parse_traj(req._traj, i) for i in range(written)], reason
