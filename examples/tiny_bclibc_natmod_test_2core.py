    ...
    try:
        ...
        try:
            ...
            if len(rows) >= 2:
                ...
                test_results['passed'] += 1
                print(f"[Core1]   {'dist_ft':>8}  {'vel_fps':>8}  {'height_ft':>8}  {'mach':>8}")
                for r in rows:
                    print(f"[Core1]   {r[1]:>8.0f}  {r[2]:>8.1f}  {r[4]:>8.3f}  {r[3]:>8.3f}")
            else:
        ...
        try:
            ...
            _pass(f"find_zero_angle elev_rad={elev:.6f}  ({math.degrees(elev):.4f} deg)")
            test_results['passed'] += 1
        except Exception as ex:
        ...
        try:
            ...
            _pass(f"find_zero_angle 100m  elev_rad={elev_100m:.6f}  ({math.degrees(elev_100m):.4f} deg  {elev_100m_mrad:.2f} mrad)")
            test_results['passed'] += 1
            if elev_100m <= 0:
                _fail("find_zero_angle 100m", f"elevation must be positive, got {elev_100m}")
                test_results['failed'] += 1
        except Exception as ex:
        ...
        try:
            ...
            _pass(f"integrate 3 km — {len(rows_3km)} rows  alloc={delta} B  alloc_after_gc={delta_gc} B")
            test_results['passed'] += 1
            print(f"[Core1]   mem_before={mem_before} B  mem_peak={mem_after} B  mem_after_gc={mem_after_gc} B")
        except Exception as ex:
...
def main():
    ...
    print(f"  Platform: RP2040 (dual-core)")
    print(f"  Zero distance: 300 m")
