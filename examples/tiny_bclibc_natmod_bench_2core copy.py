...
def run_benchmarks_on_core1():
    ...
    try:
        ...
        print(f"[Core1]   Free memory: {results['memory']['mem_free_before']:,} B → {results['memory']['mem_free_after']:,} B")
...
def main():
    ...
    print(f"Running on: RP2040 (dual-core)")
    print()
    ...
    if benchmark_error:
        ...
        if 'integrate_at' in benchmark_result:
            ...
            print(f"  integrate_at(): {r['avg_ms']:.3f} ms, ~{r['calls_per_sec']:.0f} calls/sec")
        
        if 'find_zero_angle' in benchmark_result:
            r = benchmark_result['find_zero_angle']
            print(f"  find_zero_angle(): {r['avg_ms']:.3f} ms, {math.degrees(r['elev_rad_avg']):.4f}°")
    else:
